...
1. colleges:
   ...
   - year (INTEGER) -- The admission year (e.g. 2024, 2025, or 2026)
...
3. cutoffs:
   ...
   - round (INTEGER) -- E.g. 1, 2, 3, or 0 (round 0 represents Mock Round 1)
...
- If you have all the information, output your final answer directly to the user in a friendly, helpful format (markdown).
- Be careful with course names and college names: use LIKE '%name%' for fuzzy matching.
- ALWAYS FILTER BY YEAR in your queries! E.g. if the user asks about 2026, add `year = 2026` to the WHERE clause. If they ask about 2025, use `year = 2025`. If they ask for a comparison, query both years or perform a join/group by.
- If the year is not specified, default to 2026.
