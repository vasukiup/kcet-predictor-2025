...
declare module 'vscode' {
	...
	export namespace antigravityExtensibility {
		...
		/**
		 * Options for sending content to the agent panel.
		 */
		export interface SendToAgentPanelOptions {
			/** Text message to send to the agent. */
			message?: string;
			/** File URIs to mention, with optional line ranges (0-indexed, inclusive). */
			files?: Array<{
				/** The file URI to mention. */
				uri: Uri;
				/** The 0-indexed start line of the selection. */
				startLine?: number;
				/** The 0-indexed end line of the selection, inclusive. */
				endLine?: number;
			}>;
			/**
			 * Whether to automatically send the message after populating the input box.
			 * When `true` (default), the message is sent immediately.
			 * When `false`, the message is placed in the input box for the user to
		...
		/**
		 * Sends a message and/or file mentions to the agent panel.
		 * Opens the panel if it is not already visible.
		 *
		 * @param options The content to send to the agent panel.
		 * @returns A thenable that resolves when the content has been sent.
		 */
		export function sendToAgentPanel(options: SendToAgentPanelOptions): Thenable<void>;
		/**
		 * Refreshes the list of MCP servers.
		 * @returns A thenable that resolves when the refresh is complete.
		 */
		export function refreshMcpServers(): Thenable<void>;

		/**
		 * Writes the content of mcp_config.json for the given plugin.
		 * @param pluginName The name of the plugin.
		 * @param content The JSON string content to write.
		 * @returns A thenable that resolves when the file has been written.
		 */
		export function writeMcpConfig(pluginName: string, content: string): Thenable<void>;

		/**
		 * Gets the content of mcp_config.json for the given plugin.
		 * @param pluginName The name of the plugin.
		 * @returns A thenable that resolves to the JSON string content of the file, or empty string if it doesn't exist.
		 */
		export function getMcpConfig(pluginName: string): Thenable<string>;
	}
