...
exports.performBuild = exports.performCheck = exports.isInBuildMode = void 0;
const rule_groups_1 = require("../../common/rule_groups");
const configured_checker_1 = require("../../common/configured_checker");
const ts = require("typescript");
const report_1 = require("./report");
const utils_1 = require("./utils");
/** Check if tsec is invoked in the build mode. */
function isInBuildMode(cmdArgs) {
...
exports.isInBuildMode = isInBuildMode;
/** Perform security checks on a single project. */
function performCheck(program, host) {
    const { checker, errors } = (0, configured_checker_1.getConfiguredChecker)(program, host);
    ...
    for (const sf of program.getSourceFiles()) {
        ...
        if (sf.isDeclarationFile)
            continue;
        const tsecErrors = checker.execute(sf).map(failure => failure.toDiagnosticWithStringifiedFixes());
        errors.push(...tsecErrors);
    }
    return errors;
}
exports.performCheck = performCheck;
const ALL_TSEC_RULE_NAMES = new Set(rule_groups_1.ENABLED_RULES.map(r => r.RULE_NAME));
...
function performBuild(args) {
    ...
    ts.parseBuildCommand;
    const { buildOptions, projects, errors } = parseBuildCommand(args);
    const reportDiagnostics = (0, report_1.createDiagnosticsReporter)(buildOptions);
    if (errors.length !== 0) {
    ...
    const allTsecErrors = [];
    const builderErrors = [];
    const builderHost = ts.createSolutionBuilderHost(ts.sys, instrumentedCreateProgram, 
    ...
    const errorCount = reportDiagnostics([...builderErrors, ...allTsecErrors], /*withSummary*/ true);
    return errorCount;
    /**
     ...
     */
    function instrumentedCreateProgram(rootNames, options, host, oldProgram, configFileParsingDiagnostics, projectReferences) {
        var _a;
        ...
        const tsecErrorsInThisProgram = performCheck(program, host !== null && host !== void 0 ? host : ts.createCompilerHost(program.getCompilerOptions()));
        allTsecErrors.push(...tsecErrorsInThisProgram);
        const tsecErrorsByFile = new Map();
        for (const error of tsecErrorsInThisProgram) {
            const fileName = (_a = error.file) === null || _a === void 0 ? void 0 : _a.fileName;
        ...
        return Object.assign(Object.assign({}, (0, utils_1.createProxy)(builderProgram)), { emit: instrumentedEmit });
        /**
         * Instrumented emit function. JS code won't be emitted for a file if there
         * is any tsec error in it.
         */
        function instrumentedEmit(targetSourceFile, writeFile, cancellationToken, emitOnlyDtsFiles, customTransformers) {
            if (targetSourceFile === undefined) {
                ...
                const tsecErrorsInThisFile = tsecErrorsByFile.get(targetSourceFile.fileName);
                if (tsecErrorsInThisFile === null || tsecErrorsInThisFile === void 0 ? void 0 : tsecErrorsInThisFile.length) {
                    return { emitSkipped: true, diagnostics: tsecErrorsInThisFile };
                }
            }
            return builderProgram.emit(targetSourceFile, writeFile, cancellationToken, emitOnlyDtsFiles, customTransformers);
        }
    }
}
exports.performBuild = performBuild;//# sourceMappingURL=https://main.vscode-cdn.net/sourcemaps/e0b7a2bcf575cfba10528c4e7c10bd3ce2d7769a/node_modules/tsec/lib/packages/tsec/build.js.map