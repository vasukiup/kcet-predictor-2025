...
const error_code_1 = require("../../common/third_party/tsetse/error_code");
const utils_1 = require("./utils");
function diagnosticToCodeFixActions(d) {
    var _a;
    const codeActions = [];
    for (let i = 0; i < ((_a = d.fixes) === null || _a === void 0 ? void 0 : _a.length) || 0; i++) {
        codeActions.push({
            fixName: 'Tsec fix',
            description: `Apply tsec fix ${i + 1}`,
...
class TsecLanguageServicePlugin {
    constructor(info) {
        this.info = info;
        this.boundGetSemanticDiagnostics = (fileName) => {
            ...
            const { checker } = (0, configured_checker_1.getConfiguredChecker)(program, this.project);
        ...
        this.boundGetCodeFixesAtPosition = (fileName, start, end, errorCodes, formatOptions, userPreferences) => {
            var _a;
            const prior = this.oldService.getCodeFixesAtPosition(fileName, start, end, errorCodes, formatOptions, userPreferences);
            const fixes = [...prior];
            const codeActionsForCurrentFile = this.codeFixActions.get(fileName);
            if (codeActionsForCurrentFile) {
                const actions = (_a = codeActionsForCurrentFile.get(computeKey(start, end))) !== null && _a !== void 0 ? _a : [];
                fixes.push(...actions);
            }
            return fixes;
        };
...
function init(modules) {
    ...
    if (tsModule.codefix) {
        tsModule.codefix.registerCodeFix({
            errorCodes: [error_code_1.ErrorCode.CONFORMANCE_PATTERN],
    ...
    return {
        create(info) {
            ...
            const proxy = (0, utils_1.createProxy)(oldService);
...
module.exports = init;//# sourceMappingURL=https://main.vscode-cdn.net/sourcemaps/e0b7a2bcf575cfba10528c4e7c10bd3ce2d7769a/node_modules/tsec/lib/packages/tsec/language_service_plugin.js.map