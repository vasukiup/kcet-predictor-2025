...
async function download(opts) {
  ...
  // This overrides to use an internal mirror of electron. To update it, you can use (for example):
  // macOS: gcloud artifacts generic upload --source ~/Library/Caches/electron/f2efda983949efe9c98cfb8b1787b50685ac72a967f6d9334cb2d6575077d327/electron-v37.3.1-darwin-arm64.zip --package=electron-darwin-arm64 --version=37.3.1 --repository=electron-mirror --project=windsurf-team --location=us-west1
  // NOTE: This electron cache mirror is not currently used and might break existing dev flow
  // once electron updates, but we keep it here for reference.
    ...
    downloadOpts = {
      ...
      mirrorOptions: {
        resolveAssetURL: (opts) => {
          const url = `https://artifactregistry.googleapis.com/download/v1/projects/windsurf-team/locations/us-west1/repositories/electron-mirror/files/electron-${opts.platform}-${opts.arch}%3A${opts.version.replace("v", "")}%3A${opts.artifactName}-${opts.version}-${opts.platform}-${opts.arch}.zip:download?alt=media`;
          console.log("Overriding electron to use gcloud mirror:", url);
          return url;
        }
      },
      unsafelyDisableChecksums: true
  ...
  return await downloadArtifact({...downloadOpts, unsafelyDisableChecksums: true});
}
