import json
import re
import sys
import os
import difflib

sys.stdout.reconfigure(encoding='utf-8')

# Ensure e:/Antigravity is in path
sys.path.append("e:/Antigravity")
from standardize_course_names import standardize_single_name, MANUAL_OVERRIDE_MAP
from merge_zero_intake_duplicates import courses_are_compatible

# Aided/Unaided split overrides and other exact overrides (aligned with 2025 where possible)
MANUAL_OVERRIDES = {
    "sapthagiri nps university": "E108",
    "sapthagiri nps univeristy": "E108",
    "sir m v school of architecture,hunasamaranahalli, bangalore": None,
    "school of planning and architecture, university of mysore": None,
    "school of planning and architchure, university of mysore": None,
    "sjb school of achitecture & planning, uthrahalli road, kengeri, bangalore": None,
    "sjb school of architecture & planning, uthrahalli road, kengeri, bangalore": None,
    "sri siddhartha school of engineering, tumkur": "E081",
    "sri siddhartha institute of technology": "E017",
    "pes university": "E009",
    "pes university (electronic city campus)": "E141",
    "pes university(electronic city campus)": "E141",
    "s.j b. institute of technology, bangalore (autonomous)": "E115",
    "university of mysuru": "E283",
    "visvesvaraya technological university, vtu, cpgs, mysuru": "E290",
    "visvesvaraya technological university,vtu,cpgs,mysuru.": "E290",
    "cauvery college of engineering": "E314",
    "mysore college of engineering and management,mysore": "E252",
    "university of visvesvaraya college of engineering": "E001",
    "university of visvesvaraya college of engineering bangalore": "E001",
    
    # 2024 Seat Matrix Specific Name Overrides
    "m s ramaiah institute of technology, bangalore": "E006",
    "m s ramaiah institute of technology, bangalore (autonomous)": "E006",
    "dayananda sagar college of engineering, bangalore": "E007",
    "dayananda sagar college of engineering, bangalore (autonomous)": "E007"
}

AIDED_UNITS = {
    "b m s college of engineering, basavanagudi, bangalore (autonomous)": {
        "B": "E003",
        "C": "E048",
        "E": "E048"
    },
    
    # Basaveshwara College Variations
    "b v v sangha's basaveshwara engineering college, bagalkote (autonomous)": {
        "B": "E031",
        "C": "E049",
        "E": "E049"
    },
    "b v v sangha's basaveshwara engineering college, bagalkote": {
        "B": "E031",
        "C": "E049",
        "E": "E049"
    },
    "b v v sangha's basaveshwara engineering college , bagalkote (autonomous)": {
        "B": "E031",
        "C": "E049",
        "E": "E049"
    },
    "b v v sangha's basaveshwara engineering college , bagalkote": {
        "B": "E031",
        "C": "E049",
        "E": "E049"
    },
    "b v v sangha`s basaveshwara engineering college , bagalkote": {
        "B": "E031",
        "C": "E049",
        "E": "E049"
    },
    
    # Ambedkar College Variations
    "dr. ambedkar institute of technology, bangalore (autonomous)": {
        "B": "E004",
        "C": "E060",
        "E": "E060"
    },
    "dr. ambedkar institute of technology, bangalore": {
        "B": "E004",
        "C": "E060",
        "E": "E060"
    },
    
    # Malnad College Variations
    "malnad college of engineering, hassan (autonomous)": {
        "B": "E024",
        "C": "E047",
        "E": "E047"
    },
    "malnad college of engineering, hassan": {
        "B": "E024",
        "C": "E047",
        "E": "E047"
    },
    
    # PDA College Variations
    "p d a college of engineering, gulbarga (autonomous)": {
        "B": "E041",
        "C": "E059",
        "E": "E059"
    },
    "p d a college of engineering, gulbarga": {
        "B": "E041",
        "C": "E059",
        "E": "E059"
    },
    
    # PES Mandya Variations
    "p e s college of engineering, mandya (autonomous)": {
        "B": "E023",
        "C": "E058",
        "E": "E058"
    },
    "p e s college of engineering, mandya": {
        "B": "E023",
        "C": "E058",
        "E": "E058"
    },
    
    # SJCE Variations
    "sri jayachamarajendra college of engineering (constituent college of jss science & technology university), Mysore": {
        "B": "E021",
        "C": "E284",
        "E": "E284"
    },
    "sri jayachamarajendra college of engineering (constituent college of jss science & technology university), mysore": {
        "B": "E021",
        "C": "E284",
        "E": "E284"
    },
    "sri jayachamarajendra college of engineering, mysore": {
        "B": "E021",
        "C": "E284",
        "E": "E284"
    },
    
    # NIE Mysore Variations
    "the national institute of engineering, Mysore (autonomous)": {
        "B": "E022",
        "C": "E056",
        "E": "E056"
    },
    "the national institute of engineering, mysore (autonomous)": {
        "B": "E022",
        "C": "E056",
        "E": "E056"
    },
    "the national institute of engineering, mysore": {
        "B": "E022",
        "C": "E056",
        "E": "E056"
    },
    
    "university b.d.t college of engineering, davanagere": {
        "A": "E061",
        "Z": "E066",
        "E": "E061"
    }
}

CUTOFF_TO_SM_COURSE_MAP = {
    "computers": "COMPUTER SCIENCE AND ENGINEERING",
    "civil": "CIVIL ENGINEERING",
    "mechanical": "MECHANICAL ENGINEERING",
    "electrical": "ELECTRICAL & ELECTRONICS ENGINEERING",
    "electronics engineering": "ELECTRONICS AND COMMUNICATION ENGG",
    "electronics and communication engineering": "ELECTRONICS AND COMMUNICATION ENGG",
    "info.science": "INFORMATION SCIENCE AND ENGINEERING",
    "information science and engineering": "INFORMATION SCIENCE AND ENGINEERING",
    "artificial intelligence": "COMPUTER SCIENCE AND ENGINEERING (AIML)",
    "aiml": "COMPUTER SCIENCE AND ENGINEERING (AIML)",
    "comp. sce. and eng (aiml)": "COMPUTER SCIENCE AND ENGINEERING (AIML)",
    "comp.sce.and eng (art.int)": "COMPUTER SCIENCE AND ENGG (ARTIFICIAL INTELLIGENCE)",
    "cs (artificial intel.)": "COMPUTER SCIENCE AND ENGG (ARTIFICIAL INTELLIGENCE)",
    "cs.and busi.sys": "COMPUTER SCIENCE AND BUSINESS SYSTEMS",
    "comp. sc. and bus sys.": "COMPUTER SCIENCE AND BUSINESS SYSTEMS",
    "cs.eng (dat ana)": "B.TECH IN COMPUTER SICENCE AND ENGG (DATA ANALYTICS)",
    "cs nw": "BTECH IN COMPUTER SCIENCE AND ENGINEERING (NETWORKS)",
    "computer science and engineering": "COMPUTER SCIENCE AND ENGINEERING",
    "aeronaut. engineering.": "AERONAUTICAL ENGINEERING",
    "aeronaut.engineering": "AERONAUTICAL ENGINEERING",
    "aerospace engineering": "AERO SPACE ENGINEERING",
    "agricultural engineering": "AGRICULTURAL ENGINEERING",
    "architecture": "ARCHITECTURE",
    "artificial intel, data sc": "ARTIFICIAL INTELLIGENCE AND DATA SCIENCE",
    "artificial intelligence and data science": "ARTIFICIAL INTELLIGENCE AND DATA SCIENCE",
    "artificial intelligence and machine learning": "ARTIFICIAL INTELLIGENCE AND MACHINE LEARNING",
    "auto. and robot.": "AUTOMATION AND ROBOTICS",
    "automobile engineering": "AUTOMOBILE ENGINEERING",
    "bio-medical engineering": "BIO-MEDICAL ENGINEERING",
    "bio-technology": "BIO-TECHNOLOGY",
    "ceramics": "CERAMICS & CEMENT ENGINEERING",
    "chemical": "CHEMICAL ENGINEERING",
    "civil engineering": "CIVIL ENGINEERING",
    "civil engineering (kannada medium)": "CIVIL ENGINEERING (KANNADA MEDIUM)",
    "civil environment engineering": "CIVIL ENVIRONMENTAL ENGINEERING",
    "civil eng. with comp.appl": "CIVIL ENGINEERING WITH COMPUTER APPLICATION",
    "computer science": "COMPUTER SCIENCE",
    "computer science and technology": "COMPUTER SCIENCE AND TECHNOLOGY",
    "computer science and technology (big data)": "COMPUTER SCIENCE AND TECHNOLOGY(BIG DATA)",
    "computer science and technology (devops)": "COMPUTER SCIENCE AND TECHNOLOGY(DEV OPS)",
    "computer and comm. engineering.": "COMPUTER AND COMMUNICATION ENGINEERING",
    "const. tech. management.": "CONSTRUCTION TECHNOLOGY AND MGMT",
    "cs and arti int": "B.TECH IN COMPUTER SCIENCE AND ARTIFICIAL INTELLIGENCE",
    "design engineering": "ENGINEERING DESIGN",
    "elec. inst. engineering": "ELECTRONICS AND INSTRUMENTATION ENGINEERING",
    "elec. telecommn. engineering.": "ELECTRONICS AND TELECOMMUNICATION ENGINEERING",
    "elect engineering. cs.": "B.TECH IN ELECTRICAL ENGINEERING AND COMPUTER SCIENCE",
    "electrical and computer": "ELECTRICAL & COMPUTER ENGINEERING",
    "electronics and comput": "ELECTRONICS & COMPUTER SCIENCE",
    "electronics and computer engineering": "ELECTRONICS & COMPUTER ENGINEERING",
    "electronics, instr. tech.": "ELECTRONICS & INSTRUMENTATION TECH.",
    "engineering design": "ENGINEERING DESIGN",
    "environmental engineering": "ENVIRONMENTAL ENGINEERING",
    "ind. engineering. management.": "INDUSTRIAL ENGINEERING & MANAGEMENT",
    "ind.prodn.": "INDUSTRIAL & PRODUCTION ENGINEERING",
    "information science and technology": "INFORMATION SCIENCE & TECHNOLOGY",
    "information technology": "INFORMATION TECHNOLOGY",
    "marine engineering": "MARINE ENGINEERING",
    "mathematics and computing": "MATHAMATICS AND COMPUTING",
    "mechanical engineering": "MECHANICAL ENGINEERING",
    "mechanical engineering (kannada medium)": "MECHANICAL ENGINEERING (KANNADA MEDIUM)",
    "mechatronics engineering": "MECHATRONICS",
    "med.elect.": "MEDICAL ELECTRONICS ENGINEERING",
    "mining engineering": "MINING ENGINEERING",
    "petroleum engineering": "PETROLEUM ENGINEERING",
    "pharm. engineering.": "BTECH IN PHARMACEUTICAL ENGINEERING",
    "polymer tech.": "POLYMER SCIENCE & TECHNOLOGY",
    "robotics engineering": "B TECH IN ROBOTIC ENGINEERING",
    "robotics and artificial intelligence": "ROBOTICS AND ARTIFICIAL INTELLIGENCE",
    "robotics and automation": "ROBOTICS AND AUTOMATION",
    "silk tech.": "SILK TECHNOLOGY",
    "telecommn.": "TELECOMMUNICATION ENGINEERING",
    "textiles technology": "TEXTILES TECHNOLOGY",
    "vlsi design and technology": "ELECTRONICS ENGINEERING(VLSI DESIGN & TECHNOLOGY)",
    "ec.eng.vsli emd": "B.TECH IN ELECTRONICS ENGINEERING (VLSI AND EMBEDDED SYSTEM)"
}

def clean_name(name):
    n = name.lower()
    n = re.sub(r'[^a-z0-9]', ' ', n)
    n = re.sub(r'\s+', ' ', n).strip()
    
    # Join consecutive single-letter words (e.g. "s j m" -> "sjm", "m s" -> "ms")
    while re.search(r'\b([a-z])\s+([a-z])\b', n):
        n = re.sub(r'\b([a-z])\s+([a-z])\b', r'\1\2', n)
        
    n = n.replace('engineering', 'engg')
    n = n.replace('technology', 'tech')
    n = n.replace('autonomous', '')
    n = n.replace('constituent college of vtu', 'vtu')
    n = n.replace('university of visvesvaraya college of engg', 'uvce')
    n = n.replace('univesity of visvesvaraya college of engg', 'uvce')
    n = n.replace('university visvesvaraya college of engg', 'uvce')
    n = n.replace('ramanagaram', 'ramanagara')
    n = n.replace('kushalanagar', 'kushalnagar')
    n = n.replace('chikaballapura', 'chikkaballapur')
    n = n.replace('chikkaballapura', 'chikkaballapur')
    n = n.replace('chikballapur', 'chikkaballapur')
    return n.strip()

GENERIC_WORDS = {
    'government', 'engg', 'college', 'technology', 'tech', 'institute', 
    'of', 'and', 'management', 'private', 'aided', 'unaided', 'autonomous', 
    'constituent', 'vtu', 'university', 'school', 'academy', 'technical', 
    'education', 'society', 'societys', 'societies', 'group', 'institutions', 
    'institution', 'cpgs', 'cpgs.', 'state', 'public', 'trust', 'trusts',
    'national', 'indian', 'regional', 'post', 'box', 'no', 'road', 'street',
    'campus', 'city', 'district', 'town', 'village',
    'bangalore', 'bengaluru', 'mysore', 'mysuru', 'mangalore', 'mangaluru', 
    'belgaum', 'belagavi'
}

def get_core_keywords(name):
    clean = clean_name(name)
    words = set(clean.split())
    core = words - GENERIC_WORDS
    core = {w for w in core if re.match(r'^[a-z0-9]+$', w)}
    return core

def run_mapping():
    print("Loading 2024 datasets...")
    with open("seat_matrix_data_2024.json", "r", encoding="utf-8") as f:
        sm_data = json.load(f)
        
    with open("round1_cutoffs_2024.json", "r", encoding="utf-8") as f:
        r1_cutoff = json.load(f)
        
    with open("round2_cutoffs_2024.json", "r", encoding="utf-8") as f:
        r2_cutoff = json.load(f)

    with open("round3_cutoffs_2024.json", "r", encoding="utf-8") as f:
        r3_cutoff = json.load(f)
        
    # Load unified course mapping
    with open("course_standardization_map.json", "r", encoding="utf-8") as f:
        std_map = json.load(f)
        
    def standardize_cutoff_dict(cutoff_dict):
        std_cutoff = {}
        for code, info in cutoff_dict.items():
            std_courses = {}
            for raw_cname, data in info["courses"].items():
                raw_lower = raw_cname.lower().strip()
                # Check CUTOFF_TO_SM_COURSE_MAP first
                if raw_lower in CUTOFF_TO_SM_COURSE_MAP:
                    std_cname = standardize_single_name(CUTOFF_TO_SM_COURSE_MAP[raw_lower])
                else:
                    matched_override = None
                    for raw_k, clean_v in MANUAL_OVERRIDE_MAP.items():
                        if raw_cname.upper().strip() == raw_k.upper().strip():
                            matched_override = clean_v
                            break
                    if matched_override:
                        std_cname = matched_override
                    elif raw_cname in std_map:
                        std_cname = standardize_single_name(std_map[raw_cname])
                    else:
                        std_cname = standardize_single_name(raw_cname)
                std_courses[std_cname] = data
            std_cutoff[code] = {
                "college_name": info["college_name"],
                "courses": std_courses
            }
        return std_cutoff

    print("Standardizing cutoff course names in memory...")
    r1_cutoff = standardize_cutoff_dict(r1_cutoff)
    r2_cutoff = standardize_cutoff_dict(r2_cutoff)
    r3_cutoff = standardize_cutoff_dict(r3_cutoff)
        
    # Pre-clean cutoff data for college matching
    cutoff_cleaned = {}
    for code, info in r1_cutoff.items():
        cutoff_cleaned[code] = {
            'name': info['college_name'],
            'clean': clean_name(info['college_name']),
            'core': get_core_keywords(info['college_name']),
            'courses': set(info['courses'].keys())
        }
        
    mapped_count = 0
    skipped_count = 0
    
    for sm_col in sm_data["colleges"]:
        name = sm_col["college_name"]
        name_lower = name.lower().strip()
        annex = sm_col.get("annexure", "C")
        
        # Check manual overrides first
        best_match = None
        for raw_k, val_code in MANUAL_OVERRIDES.items():
            if name_lower == raw_k:
                best_match = val_code
                break
                
        if best_match is not None or name_lower in MANUAL_OVERRIDES:
            pass
        elif name_lower in AIDED_UNITS:
            best_match = AIDED_UNITS[name_lower].get(annex)
        else:
            # Match dynamically
            sm_clean = clean_name(name)
            sm_core = get_core_keywords(name)
            sm_courses = {standardize_single_name(c["course_name"]) for c in sm_col["courses"]}
            
            scored = []
            for code, info in cutoff_cleaned.items():
                intersect = len(sm_core & info['core'])
                
                if intersect == 0 and len(sm_core) > 0 and len(info['core']) > 0:
                    continue
                    
                ratio = difflib.SequenceMatcher(None, sm_clean, info['clean']).ratio()
                overlap_courses = len(sm_courses & info['courses'])
                
                overlap_score = intersect * 10 + overlap_courses * 2
                if ratio >= 0.95:
                    overlap_score += 100
                scored.append((code, ratio, intersect, overlap_score))
                
            if scored:
                scored.sort(key=lambda x: (-x[3], -x[1], -x[2]))
                if scored[0][3] > 0 or scored[0][1] > 0.75:
                    best_match = scored[0][0]
                    
        if best_match is None:
            skipped_count += 1
            sm_col["kea_code"] = None
            for sm_course in sm_col["courses"]:
                sm_course["round1_cutoff"] = {}
                sm_course["round2_cutoff"] = {}
                sm_course["round3_cutoff"] = {}
            continue
            
        sm_col["kea_code"] = best_match
        mapped_count += 1
        
        # Map course cutoffs for all three rounds
        for sm_course in sm_col["courses"]:
            c_name = sm_course["course_name"]
            
            def get_cutoff_data(cutoff_dict, code, course_name):
                codes_to_try = [code]
                if code == "E056":
                    codes_to_try.append("E178")
                
                std_cname = standardize_single_name(course_name)
                target_lower = course_name.lower().strip()
                
                for c_code in codes_to_try:
                    if c_code not in cutoff_dict:
                        continue
                    
                    # 1. Try mapping CUTOFF_TO_SM_COURSE_MAP
                    for cutoff_cname, cut_data in cutoff_dict[c_code]["courses"].items():
                        cutoff_lower = cutoff_cname.lower().strip()
                        
                        mapped_target = CUTOFF_TO_SM_COURSE_MAP.get(cutoff_lower, "")
                        if mapped_target and clean_name(mapped_target) == clean_name(course_name):
                            return cut_data
                            
                        # 2. Try direct names match
                        if cutoff_lower == target_lower or clean_name(cutoff_cname) == clean_name(course_name):
                            return cut_data
                            
                        # 3. Try standardized match
                        if standardize_single_name(cutoff_cname) == std_cname:
                            return cut_data
                            
                        # 4. Try compatible match fallback
                        if courses_are_compatible(course_name, cutoff_cname) or courses_are_compatible(std_cname, cutoff_cname):
                            return cut_data
                return {}
                
            sm_course["round1_cutoff"] = get_cutoff_data(r1_cutoff, best_match, c_name)
            sm_course["round2_cutoff"] = get_cutoff_data(r2_cutoff, best_match, c_name)
            sm_course["round3_cutoff"] = get_cutoff_data(r3_cutoff, best_match, c_name)
            
    print(f"Mapped {mapped_count} colleges successfully.")
    print(f"Skipped {skipped_count} colleges.")
    
    # Save the updated matrix
    with open("seat_matrix_data_2024.json", "w", encoding="utf-8") as f:
        json.dump(sm_data, f, indent=2, ensure_ascii=False)
        
    print("Successfully rebuilt seat_matrix_data_2024.json with correct mappings!")

if __name__ == "__main__":
    run_mapping()
